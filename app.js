...
function buildClientPdfDefinition(){
  ...
  const terms=[];if(deliveryTime)terms.push(`Termin realizacji: ${deliveryTime}`);if(paymentTerms)terms.push(`Warunki płatności: ${paymentTerms}`);if(validUntil)terms.push(`Oferta ważna do: ${new Date(validUntil+'T12:00:00').toLocaleDateString('pl-PL')}`);
  return {
    pageSize:'A4',pageMargins:[34,40,34,45],defaultStyle:{font:'Roboto',fontSize:9,color:'#18212b'},
    footer:(current,pageCount)=>({text:`Idea Nova - oferta ${quoteNo} | strona ${current} z ${pageCount}`,alignment:'center',fontSize:7,color:'#687789',margin:[0,15,0,0]}),
    content:[
      {columns:[{stack:[{text:'IDEA NOVA',fontSize:20,bold:true,color:'#173b63'},{text:'Fronty aluminiowe - oferta handlowa',fontSize:9,color:'#687789'}]},{stack:[{text:quoteNo,alignment:'right',fontSize:13,bold:true},{text:`Data: ${new Date().toLocaleDateString('pl-PL')}`,alignment:'right',fontSize:8,color:'#687789'}]}]},

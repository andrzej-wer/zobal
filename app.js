...
async function loadQuote(){
  ...
  const [{data:req,error:reqErr},{data:items,error:itemErr},{data:zobals,error:zErr},{data:clientQuotes,error:cErr}]=await Promise.all([
  ...
  if(reqErr||itemErr||zErr||cErr){showMsg((reqErr||itemErr||zErr||cErr).message,'error');return}
  window.requestId=id; window.quoteRequest=req; window.quoteItems=items||[]; window.zobals=zobals||[]; window.clientQuotes=clientQuotes||[];
  renderQuote();
}
function renderQuote(){
  const r=window.quoteRequest,c=r.customers||{};
  el('title').textContent=r.request_number||'Zapytanie'; el('status').value=r.status;
  el('customer').innerHTML=`<strong>${c.name||'—'}</strong>${c.company_name?'<br>'+c.company_name:''}<br>${c.email||'—'}<br>${c.phone||'—'}<br>${[c.address,c.zip,c.city].filter(Boolean).join(', ')}`;
  el('notes').value=r.internal_notes||'';
  el('items-body').innerHTML=window.quoteItems.map(i=>`<tr><td>${i.position}</td><td>${i.item_name||''}<br><span class="muted">${i.profile_code||''} / ${i.finish||''} / ${i.filling||''}</span></td><td>${i.height_mm||'—'} × ${i.width_mm||'—'}</td><td>${i.quantity}</td><td><input type="number" step="0.01" value="${i.sale_unit_net||''}" data-item="${i.id}" class="price-input"></td><td>${money((i.sale_unit_net||0)*(i.quantity||0))}</td></tr>`).join('');
  el('zobal-list').innerHTML=window.zobals.map(z=>`<div class="doc-row internal"><div><strong>${z.zobal_number}</strong><br><span class="muted">${fmtDate(z.calculation_date||z.created_at)} · ${z.purchase_net?money(z.purchase_net)+' netto':'bez kwoty'}</span></div><div>${z.document_path?`<button class="btn btn-light" onclick="openPrivateFile('zobal-internal','${z.document_path}')">Otwórz PDF</button>`:''}</div></div>`).join('')||'<p class="muted">Brak kalkulacji Zobal.</p>';
  el('client-docs').innerHTML=window.clientQuotes.map(q=>`<div class="doc-row client"><div><strong>${q.quote_number}/V${q.version}</strong><br><span class="muted">${money(q.gross_total)} brutto · ${q.status}</span></div><div>${q.client_pdf_path?`<button class="btn btn-light" onclick="openPrivateFile('client-documents','${q.client_pdf_path}')">Otwórz PDF</button>`:''}</div></div>`).join('')||'<p class="muted">Brak dokumentów dla klienta.</p>';
}
